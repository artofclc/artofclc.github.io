...
function setup() {
  canvas = createCanvas(windowWidth, windowHeight - getControlsHeight());
...
function windowResized() {
  resizeCanvas(windowWidth, windowHeight - getControlsHeight());
}

function getControlsHeight() {
  const controls = document.getElementById('controls');
  const safetyMargin = 10; // Adjust this value as needed
  const computedStyle = window.getComputedStyle(controls);
  const marginTop = parseInt(computedStyle.marginTop, 10);
  const marginBottom = parseInt(computedStyle.marginBottom, 10);

  return controls.offsetHeight + marginTop + marginBottom + safetyMargin;
}
