...
var previousTool = ''; // variable to store the previous tool before erasing
var previousColor = '';
var usingEraser = false; // flag to check if the eraser tool is being used
...
document.getElementById('eraserButton').addEventListener('click', function () {
  ...
  previousColor = currentColor; // Save the current color before setting it to the eraser color
...
function draw() {
  if (mouseIsPressed) {
    ...
    // Reset the currentTool to the brush tool when the mouse is released
    if (usingEraser) {
      currentTool = '1';
